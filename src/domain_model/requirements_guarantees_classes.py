...
class Security_Level_IEC_62443():
    ...
    def __init__(self, security_level_type:str, system_or_component:str, data_origin:str):
        self.security_level_type:str = security_level_type
        self.system_or_component:str = system_or_component
        self.data_origin:str = data_origin
        if security_level_type == "Status":
            value = SL_Status_Enum.NODEFINITION
        else:
            # SL0 for SL-T/-C/-A
            value = 0 
        ...
        for i in range(len(list_of_lengths)):
            temp = {f"{i+1}.{j+1}" : value for j in range(list_of_lengths[i])}
            self.cr_sr.update(temp)

    def print_vector(self):
        print(self.security_level_type)
        print(self.system_or_component)
        print(self.data_origin)
        for key, value in self.cr_sr.items():
            print("CR/SR", key, "--", self.security_level_type, value)
